...
public class CommonLogController {
    ...
    @PostMapping("/page")
    public Result<?> getList(@RequestBody @Validated GetCommonLogVO vo) {
        PageBO<CommonLogDO> pageBO = new PageBO<>(vo.getPageNo(), vo.getPageSize());
        commonLogService.getByPage(vo.getBusinessTypeCode(), vo.getTitle(), vo.getBeginTime(), vo.getEndTime(), pageBO);
